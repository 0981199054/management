      ...
      <el-button @click="clearBtn" type="primary"  >清空查询</el-button>
            ...
            <el-table-column label="商品图片" width="150" prop="img">
              <template slot-scope="scope">
                  <img class="img" :src="ip+/images/+scope.row.img" alt="">
              </template>
            </el-table-column>
...
export default {
  ...
  created () {
    this.id = this.$store.state.login.list.storeAll[0]._id
    axios
      ...
      .then(msg => {
        this.total = msg.data.dingdan.length
        this.tableData3 = msg.data.dingdan
      })
    console.log(this.id)
  },
  methods: {
    ...
    decBtn () {
      if (this.multipleSelection.length === 0) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          axios.post(this.IP + '/dingdan', {_id: this.$store.state.login.list.storeAll[0]._id}).then(msg => {
            this.arr = msg.data
            for (let i = 0; i < this.multipleSelection.length; i++) {
              this.arr.dingdan = this.arr.dingdan.filter(item => {
                return (
                  item !== this.multipleSelection[i]._id
                )
              })
            }
            axios.post(this.IP + '/updata', this.arr).then(msg => {
              axios.post(this.IP + '/dingdandel', {
                ids: this.multipleSelection.map(item => item._id)
              }).then(msg => {
                axios.post(this.IP + '/dingdan', {
                  _id: this.id,
                  submitType: 'findJoin',
                  ref: ['dingdan']
                }).then(msg => {
