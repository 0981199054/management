<template>
  <div class="parentDiv">
    ...
    <div class="contentDiv">
      <div>
        <label>店铺名称：</label> <input id="store_name" name="store_name" class="content_input" placeholder="请输入内容"></div>
      <div>
        <label>商品名称：</label> <input id="commodity_name" name="commodity_name" class="content_input" placeholder="请输入内容"></div>

      <div>
        <label>价格：</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input id="price" name="price" class="content_input" placeholder="请输入内容"></div>
      <div>
        <label>促销价格：</label> <input id="promotion" name="promotion" class="content_input" placeholder="请输入内容"></div>
      <div>
        <label>销量：</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input id="sales" name="sales" class="content_input" placeholder="请输入内容"></div>
      <div style="display:flex">
        <label style="fontWeight:900;">商品上市时间：</label>
        <GetTime style="marginTop:-5px" @GetTime="getTime" />
      </div>

      <div>
        <label>库存：</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input id="stock" name="stock" class="content_input" placeholder="请输入内容"></div>

      <div>
        <label>评分:</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input id="grade" name="grade" class="content_input" placeholder="请输入内容"></div>
    ...
    <div style="display:flex">
      <label style="fontWeight:900">地理位置： </label>
      <Postioncity @selsectedText="getPostioncity" />
    </div>

    <div class="sonzujian">
      <label style="fontWeight:900;">产品参数：</label>
      <parameters @childValue="getVal" />
      <span style="fontSize:13px;color:gray;marginLeft:10px">(请手动输入该商品的参数信息;输入信息会在下方展示)</span>
    </div>

    <div class="standard" style="marginTop:30px">
      <label style="fontWeight:900">规格：</label>
      <!-- 规格内容组件 -->
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="severdiv" style="marginTop:30px">
      <label style="fontWeight:900;">服务：</label>
    ...
    <!-- 详情 -->
    <div class="xiang_option">
      <label style="fontWeight:900">详情：</label>
      <parTiculars @parTiculars="getparTicu" />
    </div>
    ...
    <!-- 商品图片 -->
    <div class="shop_img">
      <label style="fontWeight:900">商品图片：</label>
      <UploadImg @uploadImgarr="getUploadimg" />
    </div>
...
export default {
  ...
  created () {
    this.thisAll = this.$store.state.login.list.storeAll[0].commodity
    this.thisId = this.$store.state.login.list.storeAll[0]._id
  },
  methods: {
    // 参数组件传值
    getVal (textext) {
    ...
    clickbtn () {
      ...
      if (
        ...
        axios.post(this.ip + '/commod/addcommdotiy', param).then(msg => {
          this.thisAll.push(msg.data)
          console.log(msg.data)
          axios
            .post(this.ip + '/commod/updateshop', {
              _id: this.thisId,
              commodity: this.thisAll
            })
            .then(msg => {
              this.$message({
                title: '成功',
